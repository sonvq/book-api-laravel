...
class ApiResponse extends Response {
    ...
    public static function errorUnauthorized($data = array(), $headers = array()) {
        return self::json(Common::failResponseFormat($data), '401', $headers);
    }
    ...
    public static function errorForbidden($data = array(), $headers = array()) {
        return self::json(Common::failResponseFormat($data), '403', $headers);
    }
    ...
    public static function errorNotFound($data = array(), $headers = array()) {
        return self::json(Common::failResponseFormat($data), '404', $headers);
    }
    ...
    public static function errorInternal($data = array(), $headers = array()) {
        return self::json(Common::failResponseFormat($data), '500', $headers);
    }
    
    public static function successResponse($data = array(), $headers = array()) {
        return self::json(Common::successResponseFormat($data), '200', $headers);
    }

    public static function errorValidation($validator,  $headers = array()) {
        if (!($validator instanceof Validator)) {
            throw new Exception('Argument is not a Validator instance (' . get_class($validator) . ' found).');
        }
        $arrayMessageValidation = self::getErrorMessageValidation($validator);
        
        return self::json(Common::failResponseFormat($arrayMessageValidation), '412', $headers);
    }
